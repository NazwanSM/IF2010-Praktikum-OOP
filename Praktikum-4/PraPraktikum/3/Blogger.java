public class Blogger implements Poster {
    ...
    public void post(Content blog) {
        ...
        if (blogCount < maxBlogs) {
            blogCount++;
            blogs[blogCount - 1] = newBlog;

            for (int j = 0; j < subscriberCount ; j++) {
                subscribers[j].addNotification("Blog " + blogName + " telah mengunggah blog baru dengan judul " + newBlog.getTitle());
            }
        }
    }
    ...
    public void changePost(Content oldBlog, Content newBlog) {
        Blog oldBlogg = (Blog) oldBlog;
        Blog newBlogg = (Blog) newBlog;
        
        for (int i = 0; i < blogCount; i++  ) {
            if (blogs[i].isSame(oldBlogg)) {
                blogs[i] = newBlogg;

                for (int j = 0; j < subscriberCount ; j++) {
                    subscribers[j].addNotification("Blog " + blogName + " telah mengubah blog " + oldBlogg.getTitle() + " menjadi " + newBlogg.getTitle());
                }
            }
        }
    }
