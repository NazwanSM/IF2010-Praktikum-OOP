...
public class InventoryManager<A extends Barang> {
    ...
    public void addCatalog(A item, int jumlah) {
        CatalogBarang<A> newCatalog = new CatalogBarang<>(item, jumlah);
        this.inventory.add(newCatalog);
    }
    ...
    public void printTotalType() {
        ...
        for (CatalogBarang<A> catalog : inventory) {
            if (catalog.barang() instanceof Baju) {
                totalBaju++;
            } else if (catalog.barang() instanceof Handphone) {
                totalHandphone++;
            }
        }
    ...
    public int getTotalAmount(String itemType) {
        ...
        int totalAmount = 0;
        for (CatalogBarang<A> catalog : inventory) {
            if (itemType.equals("Baju") && catalog.barang() instanceof Baju) {
                totalAmount += catalog.jumlah();
            } 
            else if (itemType.equals("Handphone") && catalog.barang() instanceof Handphone) {
                totalAmount += catalog.jumlah();
            }
        }
        return totalAmount;
    ...
    public int getTotalPrice() {
        int totalPrice = 0;
        for (CatalogBarang<A> catalog : inventory) {
            totalPrice += catalog.barang().getPrice() * catalog.jumlah();
        }
        return totalPrice;
    }
    ...
    public void printInventory() {
        if (inventory.isEmpty()) {
            ...
            for (CatalogBarang<A> catalog : inventory) {
                String name = catalog.barang().getName();
                int price = catalog.barang().getPrice();
                int amount = catalog.jumlah();
