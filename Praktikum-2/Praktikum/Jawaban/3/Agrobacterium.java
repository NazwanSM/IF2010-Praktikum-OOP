...
public class Agrobacterium extends Bacteria implements AdaptiveResponse, ToxicAttack {
    ...
    public void toxicAttack(BacteriaBehaviour target) {
        int targetPopulation = target.getCurrentPopulation();
        int newPopulation = Math.max(0, targetPopulation - toxinLevel);
        target.setCurrentPopulation(newPopulation);
    }
    ...
    public void doAdaptiveResponse() {
        double newGrowthRate = getGrowthRate() * 1.3;
