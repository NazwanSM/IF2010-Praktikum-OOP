...
public abstract class Boss {
    ...
    public static void playPantheon(Knight knight) {
        ...
        for (int i = 0; i < pantheonSize; i++) {
            ...
            int attackPower = boss.getAttackPower();
            if (boss != null) {
                if (boss instanceof SpecialBosses) {
                    if (difficulty.equals("Attuned")) {
                        ((SpecialBosses) boss).specialAttack(knight);
                    } else if (difficulty.equals("Ascended")) {
                        boss.setAttackPower(attackPower + 20);
                        ((SpecialBosses) boss).specialAttack(knight);
                    } else if (difficulty.equals("Radiant")) {
                        boss.setAttackPower(attackPower + 30);
                        ((SpecialBosses) boss).specialAttack(knight);
                    }
                }
            }
