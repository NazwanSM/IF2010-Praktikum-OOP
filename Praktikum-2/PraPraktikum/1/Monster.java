...
public class Monster implements MonsterTarget, Comparable<Monster> {
    ...
    public void counterAttack(Hunter hunter) {
        int dmg = (int) (attackPower * (isEnraged ? ENRAGE_MULTIPLIER : 1));
        hunter.setHealth(hunter.getHealth() - dmg);

        System.out.println(name + (isEnraged ? " [ENRAGED]" : "") + " counter-attacks " + hunter.getName() + " for " + dmg + " damage!");
    ...
    public void areaAttack(Hunter[] hunters) {
        int dmg = (int) (attackPower * 0.7 * (isEnraged ? ENRAGE_MULTIPLIER : 1));
        
        System.out.println(name + (isEnraged ? " [ENRAGED]" : "") + " performs area attack for " + dmg + " damage to all hunters!");
